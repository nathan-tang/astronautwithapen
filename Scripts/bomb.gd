...
func setup_fuse_particles() -> void:
	...
	fuse_particles.amount = 8
	fuse_particles.lifetime = 0.5
	fuse_particles.explosiveness = 0.0
	...
	for result in results:
		...
		if body != self:
			...
			if body is RigidBody2D:
				...
				# For CharacterBody2D (like player), add extra force
				var player_impulse = impulse * player_explosion_multiplier
				body.velocity += player_impulse
